...
class FetchMoviService {
  ...
  // https://api.themoviedb.org/3/movie/76600/credits?api_key=9673c8c8f98cb6e489d5cad6b3789836&language=en-US
  async getFilmCast(id) {
    const { searchParams, MOVIE_ID_URL, RESPONSE_OK } = this;
    const response = await axios.get(MOVIE_ID_URL + id + '/credits', {
      params: searchParams,
    });
    if (response.status !== RESPONSE_OK) {
      throw new Error(response.status);
